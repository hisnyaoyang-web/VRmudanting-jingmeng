/**
 * VR Bootstrap - WebXR application layer for the shadow-puppet scroll game.
 * Does not modify any game source. The game runs unchanged in the DOM;
 * this module composites the game onto a canvas each frame and projects
 * it onto a VR screen. Two modes: cinema (flat screen) and immersive (curved panorama).
 * VR thumbstick maps to left/right movement.
 */
...
function captureFrame() {
  ...
  if (seam) {
    ...
    if (r.right > 0 && r.left < window.innerWidth) {
      const g = ctx.createLinearGradient(r.left * scaleX, 0, r.right * scaleX, 0);
  ...
  if (door && !door.classList.contains("hidden")) {
    ...
    g.addColorStop(0, "rgba(245,201,107,0.5)"); g.addColorStop(0.7, "rgba(245,201,107,0)");
    ctx.save(); ctx.globalCompositeOperation = "screen"; ctx.fillStyle = g;
    ctx.fillRect(r.left * scaleX, r.top * scaleY, r.width * scaleX, r.height * scaleY); ctx.restore();
  }
  ...
  if (rs) {
    for (const c of rs.querySelectorAll<HTMLCanvasElement>("canvas")) {
      ...
      if (r.width > 0 && r.height > 0) ctx.drawImage(c, r.left * scaleX, r.top * scaleY, r.width * scaleX, r.height * scaleY);
    }
  }
  ...
  if (sub && !sub.classList.contains("hidden")) {
    ...
    if (text) {
      ...
      ctx.fillStyle = "rgba(14,6,10,0.75)"; ctx.fillRect(cx - tw / 2 - 24, cy - 28, tw + 48, 56);
      ctx.fillStyle = "#f2d8a8"; ctx.shadowColor = "rgba(217,138,60,0.6)"; ctx.shadowBlur = 8;
...
// === Cinema mode objects (flat screen) ===
...
// === Immersive mode objects (curved panorama wrapping around viewer) ===
const IMMERSE_RADIUS = 3.5;
const IMMERSE_HEIGHT = 5;
const IMMERSE_ARC = Math.PI * 1.6; // ~288 degrees

// Build inward-facing curved screen with custom UV mapping
...
modeBtn.style.cssText = "position:fixed;bottom:80px;left:50%;transform:translateX(-50%);z-index:999;" +
  ...
  "border-radius:6px;padding:10px 28px;cursor:pointer;box-shadow:0 4px 18px rgba(217,138,60,.4);" +
  "pointer-events:auto;transition:all .2s;";
modeBtn.onmouseenter = () => { modeBtn.style.transform = "translateX(-50%) translateY(-2px)"; };
modeBtn.onmouseleave = () => { modeBtn.style.transform = "translateX(-50%)"; };
modeBtn.onclick = () => {
  setMode(!immersive);
...
console.log("[VR] WebXR layer ready - cinema + immersive modes");
