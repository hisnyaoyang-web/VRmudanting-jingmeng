...
export function resolveAsset(storyUrl: string, story: StoryPackage, asset?: string) {
  if (!asset) return "";
  // The original assumed an absolute https storyUrl; this port serves the story
  // from the same origin (relative path), so resolve against window.location.
  const docBase = (typeof window !== "undefined" && window.location) ? window.location.href : "file:///";
  try {
    const base = story.assetBaseUrl ? new URL(story.assetBaseUrl, docBase) : new URL(".", docBase);
...
function matches(condition: StoryCondition, state: { current: { grade: StoryGrade; scoreRatio: number }; history: CustomerHistory }) {
  ...
  if (condition.op === "tailEquals") {
    const expected = Array.isArray(condition.value) ? (condition.value as unknown[]) : [];
    if (!Array.isArray(actual) || expected.length === 0 && !Array.isArray(condition.value)) return false;
    return actual.slice(-expected.length).every((value, index) => value === expected[index]);
  }
  return false;
}
