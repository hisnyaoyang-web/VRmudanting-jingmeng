...
/**
 * 长卷模型：一整张连续画卷，左半闺房（场景1），右半花园（场景2），
 * 中间以剪纸屏柱遮挡接缝。人偶只在长卷上左右自由移动，镜头跟随平移。
 *
 * 全程无按键：所有剧情（敲门 / 对话 / 奖励 / 拼合 / 结算）都靠
 * "走到区域 + 计时器" 自动演完。唯一保留的是开场"开始体验"与结算的"重新/结束"按钮。
 * 方向输入（左右键 / 触屏）始终生效，移动不受任何状态限制。
 */

/** 镜头跟随：人偶目标屏幕位置（视口宽比例） */
const CAM_FOLLOW = 0.42;
/** 单个场景的世界宽度（视口倍数）。与 style.css 的 --scene-w 保持一致。 */
const SCENE_W = 1.4;
/** 长卷可滚动范围 */
...
/** 自动推进节奏（秒）：剧情靠到达区域 + 计时自动演完 */
const KNOCK_INTERVAL = 1.8;   // 每次敲门台词间隔
const TALK_INTERVAL = 3.2;    // 每句花园对话间隔
const REWARD_HOLD = 3.6;      // 奖励卡片自动停留
const LEAVE_TEXT_HOLD = 4.5;  // "不到园林" 停留后给双手
...
type GState =
  ...
  | "toTheatre" | "rhythm" | "rhythmDone" | "gardenRoam"
  | "gardenTalk" | "rewardFeet" | "toMirror" | "assemble" | "rewardTorso"
  | "finale";
...
const GARDEN_TALK: { who: "玩家" | "杜丽娘"; text: string; anim?: PuppetState }[] = [
  { who: "玩家", text: "你是谁？" },
  { who: "杜丽娘", text: "我是杜丽娘。", anim: "hi" },
  { who: "玩家", text: "你是杜丽娘，那我是谁？" },
  { who: "杜丽娘", text: "你也是杜丽娘。" },
  { who: "玩家", text: "不，我不是杜丽娘。" },
  { who: "杜丽娘", text: "请随我来。", anim: "hi" },
];
...
.mudan-overlay { position: fixed; inset: 0; z-index: 60; display: flex; align-items: center; justify-content: center;
  background: radial-gradient(ellipse 80% 70% at 50% 45%, rgba(22,10,16,.86), rgba(5,2,6,.97)); animation: mdFadeIn .5s ease; }
...
.md-primary { padding:12px 30px; font-size:clamp(16px,2vw,20px); color:#1a0a08;
...
/* 开场 */
#md-intro .md-card { max-width:min(560px,88vw); padding:38px 34px 30px; text-align:center; color:var(--paper);
...
.md-title { font-size:clamp(30px,5vw,46px); letter-spacing:.3em; text-indent:.3em; color:var(--gold-bright);
  text-shadow:0 0 24px rgba(217,138,60,.5); }
.md-sub { margin-top:6px; font-size:clamp(15px,2vw,19px); letter-spacing:.4em; text-indent:.4em; color:var(--ember); }
.md-controls { list-style:none; margin:26px auto 12px; max-width:420px; text-align:left; }
...
#md-sub.shake{ animation:mdShake .5s ease; }
@keyframes mdSubIn{ from{opacity:0;transform:translate(-50%,10px)} to{opacity:1;transform:translate(-50%,0)} }
@keyframes mdShake{ 0%,100%{transform:translateX(-50%)} 20%{transform:translate(calc(-50% - 5px),2px) skewX(-3deg)}
  40%{transform:translate(calc(-50% + 5px),-2px) skewX(3deg)} 60%{transform:translate(calc(-50% - 4px),2px) skewX(-2deg)}
  80%{transform:translate(calc(-50% + 4px),-1px) skewX(2deg)} }
/* 门暖光（位于长卷内闺房门处） */
...
.mirror-frags{ position:absolute; inset:0; display:flex; flex-direction:column; align-items:center; justify-content:space-around;
  pointer-events:none; }
.frag{ font-family:var(--serif); font-size:clamp(13px,1.1vw,18px); letter-spacing:.2em; text-indent:.2em; color:var(--paper);
  padding:4px 14px; border-radius:4px; background:linear-gradient(180deg,rgba(217,138,60,.85),rgba(180,100,40,.85));
  border:1px solid var(--gold-bright); box-shadow:0 0 12px rgba(240,192,96,.7); opacity:0; transform:scale(.6);
  transition:opacity .5s ease,transform .5s ease; }
#md-mirror.blurred .mirror-img{ filter:blur(4px) brightness(.7); }
#md-mirror.filled-hands .frag-hands,#md-mirror.filled-feet .frag-feet,#md-mirror.filled-torso .frag-torso{
...
.md-reward-card{ width:min(420px,86vw); padding:30px 28px 26px; text-align:center; color:var(--paper);
  background:linear-gradient(180deg,rgba(28,12,18,.94),rgba(14,6,10,.97)); border:1px solid rgba(217,160,63,.45);
  border-top:2px solid var(--gold); border-bottom:2px solid var(--gold);
  box-shadow:0 0 40px rgba(217,138,60,.25),0 18px 50px rgba(0,0,0,.6); animation:mdPop .5s cubic-bezier(.2,.9,.3,1.2); }
@keyframes mdPop{ from{opacity:0;transform:scale(.9)} to{opacity:1;transform:scale(1)} }
.md-shard{ width:70px; height:90px; margin:0 auto 16px;
  background:radial-gradient(circle at 40% 30%,var(--gold-bright),var(--ember) 60%,var(--lacquer));
  clip-path:polygon(50% 0,100% 35%,80% 100%,20% 100%,0 35%); box-shadow:0 0 26px rgba(240,192,96,.6);
  animation:mdGlow 2.4s ease-in-out infinite; }
@keyframes mdGlow{ 0%,100%{filter:brightness(1)} 50%{filter:brightness(1.3)} }
...
#md-ripple{ position:fixed; inset:0; z-index:65; pointer-events:none;
  ...
  animation:mdRipple 2.2s ease forwards; }
...
/* 结算 */
#md-finale .md-finale-card{ width:min(480px,88vw); padding:34px 30px 28px; text-align:center; color:var(--paper);
...
export class FlatStage {
  ...
  private theatreWrap: HTMLElement | null = null;
  ...
  // 状态机（全程无按键：靠走到区域 + 计时自动推进）
  private state: GState = "intro";
  private timer = 0;          // 当前状态计时
  private knockCount = 0;     // 已敲门次数
  ...
  constructor(stage: HTMLElement) {
    ...
    this.subtitleText = this.subtitleEl;
    this.rewardEl = this.inject(`<div id="md-reward" class="hidden">
      <div class="md-reward-card"><div class="md-shard"></div><h2></h2><p></p></div></div>`);
    ...
    this.finaleEl = this.inject(`<div id="md-finale" class="mudan-overlay hidden">
      <div class="md-finale-card"><h2 class="md-finale-name">你是___杜丽娘</h2>
      <p class="md-finale-type"></p><p class="md-finale-keys"></p><p class="md-finale-text"></p>
      <div class="md-finale-btns"><button class="md-btn md-primary" type="button">重新体验</button>
      <button class="md-btn md-ghost" type="button">结束体验</button></div></div></div>`);
    ...
    this.introEl = this.inject(`<div id="md-intro" class="mudan-overlay">
      <div class="md-card"><h1 class="md-title">梦入牡丹亭</h1><p class="md-sub">寻回杜丽娘</p>
      <ul class="md-controls">
      <li><span class="md-ctl-keys"><kbd>←</kbd><kbd>→</kbd></span><span>左右移动，走到目标即可</span></li>
      </ul><p class="md-note">剧情会自动展开，无需按键。</p>
      <button class="md-btn md-primary" type="button">开始体验</button></div></div>`);

    // 门光、镜子放进长卷内（随镜头平移）
    this.doorGlowEl = this.inject(`<div id="md-door" class="hidden" aria-hidden="true"></div>`, this.panorama);
    this.mirrorEl = this.inject(`<div id="md-mirror"><img class="mirror-img" src="/mirror/mirror.png" alt="" /><div class="mirror-frags"><span class="frag frag-hands">双手</span><span class="frag frag-feet">双脚</span><span class="frag frag-torso">躯干</span></div></div>`, this.panorama);
  ...
  private beginGame() {
    ...
    this.startBgm();
    this.enter("approach");
    this.showHint(true, 4500);
  }

  private startBgm() {
    if (!this.bgmAudio) {
      this.bgmAudio = new Audio("/audio/chamber/act1-bgm.mp3");
  ...
  private say(text: string, who?: string, shake = false) {
    const whoHtml = who ? `<span class="who">${who}</span>` : "";
    this.subtitleText.innerHTML = whoHtml + text;
    this.subtitleEl.classList.remove("hidden", "shake");
    if (shake) { void this.subtitleEl.offsetWidth; this.subtitleEl.classList.add("shake"); }
  }
  ...
  private enter(s: GState) {
    ...
    switch (s) {
      ...
      case "freeRoam":
        this.hideReward();
        this.say("穿过屏柱，前方似有戏台……", undefined);
        break;
      case "toTheatre":
        this.clearSay();
        this.ensureTheatre();
        this.say("月门之内，灯影将起——", "行山");
        break;
      case "rhythm":
        this.clearSay();
        break;
      case "rhythmDone":
        this.hideTheatre();
        this.say("一曲既罢，再向前便是花园。", undefined);
        break;
      case "gardenRoam":
        this.clearSay();
        break;
     ...
     case "toMirror":
       this.hideReward();
        this.mirrorEl.classList.remove("blurred", "filled-hands", "filled-feet", "filled-torso");
       this.say("镜中似有人影，走近看看。", "花园");
       break;
      case "assemble":
        this.clearSay();
        this.mirrorEl.classList.add("blurred");
  ...
  private ensureTheatre() {
   ...
   this.theatreEl = wrap;
   this.theatreWrap = wrap;
    this.enter("rhythm");
   // 挂载原版音游（React + R3F）：舞台、月门背景、皮影、下落音符、键盘评分全部沿用原版
    this.rhythmRoot = mountRhythm(wrap, {
      onFinish: (res) => {
        this.rhythmGrade = res.grade;
        this.enter("rhythmDone");
      },
    });
    this.rhythmMounted = true;
  ...
  private startFinale() {
    ...
    const nameEl = this.finaleEl.querySelector(".md-finale-name") as HTMLElement;
    nameEl.textContent = "你是「镜中人」杜丽娘";
    this.finaleType.textContent = "结果类型 · 镜中人";
    this.finaleKeys.textContent = "关键词：寻回 · 觉醒 · 自由";
    this.finaleText.textContent =
      "双手织梦，双脚择路，躯干里盛着看客的影子。你寻回的杜丽娘，从来都住在你自己身上。";
  }
  private replay() {
   this.finaleEl.classList.add("hidden");
   this.clearSay();
   this.mirrorEl.classList.remove("filled-hands", "filled-feet", "filled-torso", "blurred");
    this.doorGlowEl.classList.remove("lit");
    ...
    this.rhythmGrade = "";
    if (this.theatreEl) { unmountRhythm(this.rhythmRoot); this.rhythmRoot = null; this.rhythmMounted = false; this.theatreEl.remove(); this.theatreEl = null; }
    this.charX = CHAMBER.start;
    this.facing = 1;
    this.actTitle("第一幕 · 幽闭闺房");
    this.enter("approach");
  ...
  update(dt: number) {
    ...
    // 戏台期间交由原版音游（React 应用）接管，暂停长卷移动与人偶渲染
    const inTheatre = this.state === "toTheatre" || this.state === "rhythm" || this.state === "rhythmDone";
    if (!inTheatre) {
      this.handleFreeMove(dt);
      this.tickState(dt);
      const targetCam = clamp(this.charX - CAM_FOLLOW, 0, SCROLL_MAX);
  ...
  private handleFreeMove(dt: number) {
    // S10-S11 拼合期间锁定移动
    if (this.state === "assemble" || this.state === "rewardTorso") {
      this.puppet?.setState("idle");
      return;
    }
    // S9 进入镜面触发区域后停止移动
    if (this.state === "toMirror" && Math.abs(this.charX - GARDEN.mirror) <= MIRROR_ZONE) {
  ...
  /** 靠"走到区域 + 计时"自动推进全部剧情，全程无需按键 */
  private tickState(dt: number) {
    switch (this.state) {
      // —— 第一幕：走到门口自动敲门 ——
      case "approach": {
        if (!this.knockStarted && Math.abs(this.charX - CHAMBER.door) <= DOOR_ZONE) {
          this.knockStarted = true;
          this.knockTimer = 0;
          this.knockCount = 1;
          this.say(KNOCK_LINES[0], "门外之声");
        }
        if (this.knockStarted && this.knockCount < 4) {
          this.knockTimer += dt;
          if (this.knockTimer >= KNOCK_INTERVAL) {
            this.knockTimer = 0;
            const idx = this.knockCount;
            const last = idx === 3;
            this.say(KNOCK_LINES[idx], last ? undefined : "门外之声", last);
            this.knockCount++;
            if (last) {
              this.doorGlowEl.classList.remove("hidden");
              this.doorGlowEl.classList.add("lit");
              window.setTimeout(() => this.enter("leaveText"), 1400);
            }
          }
        }
        break;
      ...
      case "leaveText": {
        this.timer += dt;
        if (this.timer >= LEAVE_TEXT_HOLD) this.enter("rewardHands");
        break;
      }
      // —— 奖励自动消失 ——
      case "rewardHands": {
      ...
      // —— 自由走向花园：过屏柱出第二幕题，到对话点开演 ——
      case "freeRoam": {
        if (!this.act2Shown && this.charX > SEAM1) {
          this.act2Shown = true;
          this.actTitle("第二幕 · 月门戏台");
        }
        if (this.charX >= THEATRE.center - THEATRE_ZONE) this.enter("toTheatre");
        break;
      }
      // —— 进入戏台：加载节奏后自动开演 ——
      case "toTheatre": {
        // 音游由挂载的 React 应用驱动，结束回调直接进入 rhythmDone
        break;
      }
      // —— 节奏演出进行中：由挂载的 React 音游自驱动 ——
      case "rhythm": {
        break;
      }
      // —— 节奏结束：短暂停留后去花园 ——
      case "rhythmDone": {
        this.timer += dt;
        if (this.timer >= 2.4) this.enter("gardenRoam");
        break;
      }
      // —— 自由走向花园对话点 ——
      case "gardenRoam": {
        ...
        if (this.charX >= GARDEN.talk - TALK_ZONE) this.enter("gardenTalk");
        break;
      }
      // —— 花园对话自动逐句 ——
      case "gardenTalk": {
      ...
      // —— S9 走近镜面：停止移动，显示模糊人影，进入 S10 ——
      case "toMirror": {
        if (Math.abs(this.charX - GARDEN.mirror) <= MIRROR_ZONE) {
          if (this.timer === 0) {
            this.mirrorEl.classList.add("blurred");
            this.say("镜中人影绰绰，似真似幻……", "花园");
          }
          this.timer += dt;
          if (this.timer >= 1.8) this.enter("assemble");
        }
        break;
      }
      // —— 拼合全自动：按计时依次填入双手 / 双脚 / 躯干 ——
      case "assemble": {
        this.timer += dt;
        if (this.timer >= 1.2 && !this.assmHands) {
          this.assmHands = true;
          this.mirrorEl.classList.add("filled-hands");
        } else if (this.timer >= 2.6 && !this.assmFeet) {
          this.assmFeet = true;
          this.mirrorEl.classList.add("filled-feet");
          this.say(ASSEMBLE_LINES.placed.text, ASSEMBLE_LINES.placed.who);
        } else if (this.timer >= 4.0 && this.mirrorEl.classList.contains("blurred")) {
          this.mirrorEl.classList.remove("blurred");
          this.say(ASSEMBLE_LINES.missing.text, ASSEMBLE_LINES.missing.who);
        } else if (this.timer >= 5.4 && !this.fragmentTorso) {
          this.fragmentTorso = true;
  ...
  private applyCamera() {
    ...
    // puppet lives inside .panorama (world space); camera scroll carries it,
    // so its left is the world X (charX * vw), not the on-screen X.
  ...
  /**
   * 场景图层准备：花园（场景2）的中后景 / 前景是绿幕抠图层，
   * 在加载时把 rgb(0,255,0) 抠成透明，注入为该半区的背景，使其与后景正确叠加。
   * 闺房（场景1）三层本身带有真实透明通道，直接 CSS 叠加即可。
   */
  private async initScenes() {
    const halves = this.panorama.querySelectorAll<HTMLElement>("[data-chroma]");
    await Promise.all(
      Array.from(halves).map((half) => this.chromaKeyLayer(half)),
    );
  }
  ...
  private async chromaKeyLayer(half: HTMLElement) {
    ...
    const d = id.data;
    // 绿幕为纯 rgb(0,255,0)；用亮度阈值抠图，保留画面里较暗的绿色植物内容。
    // g>=240 且 R/B 极低 -> 全透明；214..240 软边过渡；其余保留。
    ...
    if (blob) {
      half.style.backgroundImage = `url("${URL.createObjectURL(blob)}")`;
    }
  }
}
